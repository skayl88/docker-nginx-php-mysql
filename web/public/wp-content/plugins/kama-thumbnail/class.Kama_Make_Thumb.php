...
class Kama_Make_Thumb {
	...
	public $thumb_path;
	public $thumb_url;
	public $metadata = [];     // разные данные для дебага

	// последний экземпляр, для доступа к $width, $height и т.д.
	static $last_instance;

	// текущий домен без www. и поддоменов: www.foo.site.com >>> site.com
	static $_main_host = '';

	// миниатюр создано за поток
	...
	public function __construct( $args = array(), $src = 'notset' ){
		...
		$this->opt->allow_hosts = array_merge(
	...
	protected function _file_name_data(){
		...
		elseif( preg_match( '~\.(jpe?g|png|gif|svg|bmp)~i', $this->src, $mm ) )
			$data->ext = strtolower( $mm[1] );
		else
			$data->ext = 'png';
	...
	protected function do_thumbnail(){
		...
		if( !empty($this->metadata['stub']) ){
			$this->thumb_path = $this->_change_to_stub( $this->thumb_path );
			$this->thumb_url  = $this->_change_to_stub( $this->thumb_url );
		}
	...
	protected function make_thumbnail_Imagick( $img_string ){
		...
		try {
			...
			// Пробуем создать через GD. Пример: https://ps.w.org/wpforms-lite/assets/screenshot-2.gif
